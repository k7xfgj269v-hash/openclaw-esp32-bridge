...
OPENCLAW_AGENT_ID = os.environ.get('OPENCLAW_AGENT_ID', 'esp32-voice')
...
class OpenClawHandler(BaseHTTPRequestHandler):
    ...
    def call_openclaw_agent(self, session_id, message):
        ...
        try:
            ...
            print(f"[OpenClaw] agent: {OPENCLAW_AGENT_ID}, session: {session_id}")
            ...
            import traceback
            traceback.print_exc()
            return "调用 AI 服务失败，请检查服务器日志"

if __name__ == '__main__':
    print(f"OpenClaw Agent 服务器启动在 {SERVER_HOST}:{SERVER_PORT}")
    print(f"智能体: {OPENCLAW_AGENT_ID}, Session 前缀: {OPENCLAW_SESSION_PREFIX}")
    print(f"OpenClaw 路径: {OPENCLAW_BIN}")
    print("等待 ESP32 连接...\n")

    try:
        ThreadingHTTPServer((SERVER_HOST, SERVER_PORT), OpenClawHandler).serve_forever()
